function ContinuumStewartGoughStaticBVP % Section 2.2.3
    % Properties
    ...
    % Equations (1.1) and (1.2) ( Written as anonymous functions for brevity )
    hat =@(y)[0,-y(3) ,y(2) ;y(3) ,0,-y(1) ;-y(2) ,y(1) ,0]; inv_hat_xy =@(y)[y(3 ,2);y(1 ,3) ];

    % Main Simulation
    init_guess = [ zeros(30 ,1) ; pE(3)* ones(6 ,1) ]; % Initial guess ; the form is given by (2.12)
    global p
    ...
    % Subfunctions
    function E = CSG_BVP_Function (G) % Equation (2.13)
    EF = F; EM = M; % Begin summing forces
    ...
    for i = 1 : 6 % Loop over each compliant robot link
        ...
        ER = inv_hat_xy ( RL_shot'* RE - RL_shot *RE' ); % Equation (1.3)
        geometric_error_range = 1+5*(i -1) : 5*i;
        E( geometric_error_range ) = [Ep; ER ];

        EF = EF - nL; EM = EM - mL - cross ( RE*r(:,i), nL ); % Continue summing forces
    end

    E (31:33) = EF; E (34:36) = EM; % Force and moment summation are complete
    end

    function ys = RodODE (s,y) % Equation (2.4)
    R = reshape (y (4:12) ,3 ,3); n = y (13:15) ; m = y (16:18) ;
    v = Kse ^ -1*R.'*n + [0;0;1]; u = Kbt ^ -1*R.'*m;
    ps = R*v; Rs = R* hat (u); ns = -rho *A*g; ms = -hat (ps)*n;
